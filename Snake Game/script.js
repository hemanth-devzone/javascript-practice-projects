...
const snakeColor = "#43cea2";
const snakeBorder = "#185a9d";
...
function nextTick() {
  if (running) {
    setTimeout(() => {
      ...
      checkGameOver();
      nextTick();
    }, 110); // Slower speed: increased from 75ms to 110ms
  } else {
    displayGameOver();
  }
}
...
function drawSnake() {
  ...
  snake.forEach((snakePart, idx) => {
    ...
    if (idx === 0) {
      ...
      ctx.beginPath();
      ctx.arc(
        snakePart.x + unitSize * 0.7,
        snakePart.y + unitSize * 0.3,
        3,
        0,
        Math.PI * 2
      );
      ctx.arc(
        snakePart.x + unitSize * 0.3,
        snakePart.y + unitSize * 0.3,
        3,
        0,
        Math.PI * 2
